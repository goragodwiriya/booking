const TableManager = {
  ...
  setupColumnResizing(tableId) {
    ...
    const saveWidths = () => {
      try {
        const widths = headers.map(c => c.style.width || window.getComputedStyle(c).width);
    ...
    headers.forEach((th, idx) => {
      ...
      // Rebind handlers on TH (not on the resizer node) so they survive
      // innerHTML rewrites done by translation/template layers.
      if (th._colResizeClickHandler) {
        th.removeEventListener('click', th._colResizeClickHandler);
      }
      if (th._colResizeMouseDownHandler) {
        th.removeEventListener('mousedown', th._colResizeMouseDownHandler);
      }
      ...
      // Keep handlers on TH because TH survives while child nodes may be recreated.
      ...
      const onResizeMouseDown = (e) => {
        ...
        // Capture startX/startWidth inside each drag to avoid shared state.
        ...
        const onMouseUp = () => {
          ...
          // Keep flag set until AFTER the synthetic post-mouseup click fires.
          // In real browsers, click fires in the same event-processing batch as
          // mouseup (before any setTimeout callbacks), so setTimeout(0) is safe.
      ...
      th.addEventListener('click', onResizeClick);
      th.addEventListener('mousedown', onResizeMouseDown);
      th._colResizeClickHandler = onResizeClick;
      th._colResizeMouseDownHandler = onResizeMouseDown;
    });
  },
  ...
  async _executeRowAction(tableId, actionUrl, item, actionKey, cfg = null, triggerEl = null) {
    ...
    // Interpolate {field} templates against the current row.
    //
    // Shared by `params` and `url` so a row action can target a REST resource
    ...
    if (targetUrl) {
      if (method === 'GET' && navigates) {
        ...
        const isDownload = cfg?.download === true || cfg?.download === 'true';

        if (target === '_blank') {
          // Open external/blank targets in a new tab; keep download-style flows
          // on the current tab when explicitly requested.
          if (!isDownload) {
        ...
        if (window.RouterManager && typeof RouterManager.navigate === 'function' && dest.startsWith('/') && target !== '_blank') {
