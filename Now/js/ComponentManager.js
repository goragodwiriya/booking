const ComponentManager = {
  ...
  /**
   * Move the instance key to a new element after the template, replacing the host element.
   *
   ***Bug fixed:**`instances` is keyed with the same element when mount, but the template of
   * Components replace elements with their own roots · Page transition cleanup
   * (RouterManager) Find elements with `data-component` in the DOM and ask for `instances`.
   * with that element — which is not the original key So we get the null value and **Never called
   * `destroyed()` of any components that use templates**
   *
   * The result is every time the page is changed. The original component's timer and listener continue to work.
   * and a new one added overlay · in apps with poll API components every 15 seconds
   * Browsing ten pages equals ten times the frequency of requests without any prompts.
   *
   * @param {HTMLElement} oldElement element host who has been replaced
   * @param {HTMLElement} newElement instead, the root of the template resides in the DOM.
   */
