...
class RichTextElementFactory extends ElementFactory {
  ...
  // Runtime-only binding state — kept off the DOM so it never leaks into saved
  // content (previously tracked via data-rte-fallback-* attributes).
  static _fallbackBound = new WeakSet();
  static _iframeLoaded = new WeakSet();
  ...
  static _attachImageFallback(img) {
    if (!img || RichTextElementFactory._fallbackBound.has(img)) {
      return;
    }

    RichTextElementFactory._fallbackBound.add(img);
    // Clear any runtime flag persisted in legacy content
    img.removeAttribute('data-rte-fallback-bound');
  ...
  static _attachIframeFallback(iframe) {
    if (!iframe || RichTextElementFactory._fallbackBound.has(iframe)) {
      return;
    }

    RichTextElementFactory._fallbackBound.add(iframe);
    // Clear any runtime flags persisted in legacy content
    iframe.removeAttribute('data-rte-fallback-bound');
    iframe.removeAttribute('data-rte-fallback-loaded');
    ...
    const markLoaded = () => {
      RichTextElementFactory._iframeLoaded.add(iframe);
      fallback.hidden = true;
      fallback.setAttribute('aria-hidden', 'true');
    ...
    const showFallback = () => {
      if (RichTextElementFactory._iframeLoaded.has(iframe)) {
        return;
      }
  ...
  /**
   * Strip runtime-only media-fallback artifacts from serialized content so they
   * are never persisted. Removes injected fallback placeholders, leftover
   * data-rte-fallback-* flags (including legacy content), and un-hides media
   * that the fallback logic had temporarily hidden.
   * @param {string} html
   * @returns {string}
   */
