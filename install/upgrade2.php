    ...
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        if (!$error) {
            ...
            try {
                ...
                foreach (['username', 'id_card', 'phone', 'activatecode', 'line_uid', 'telegram_id', 'status'] as $_idx) {
                ...
                foreach (['username', 'id_card', 'phone'] as $_idx) {
                    if (!$db->indexExists($table_user, $_idx)) {
                ...
                // login_attempt
                // =========================================================
                $table_login_attempt = $db_config['prefix'].'_login_attempt';
                if (!$db->tableExists($table_login_attempt)) {
                    $db->query("CREATE TABLE IF NOT EXISTS `$table_login_attempt` (
                    ...
                    if (!$db->indexExists($table_login_attempt, 'idx_ip_time')) {
                        $db->query("ALTER TABLE `$table_login_attempt` ADD INDEX `idx_ip_time` (`ip_address`, `attempted_at`)");
                        $content[] = '<li class="correct">login_attempt: เพิ่ม index idx_ip_time</li>';
                    }
                    if (!$db->indexExists($table_login_attempt, 'idx_user_time')) {
                        $db->query("ALTER TABLE `$table_login_attempt` ADD INDEX `idx_user_time` (`username`, `attempted_at`)");
                        $content[] = '<li class="correct">login_attempt: เพิ่ม index idx_user_time</li>';
                    }
                ...
                // user_session
                // =========================================================
                $table_user_session = $db_config['prefix'].'_user_session';
                if (!$db->tableExists($table_user_session)) {
                    $db->query("CREATE TABLE IF NOT EXISTS `$table_user_session` (
